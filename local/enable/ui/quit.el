...
(defun rps-doom-quit-p (&optional prompt)
...
(defun rps-doom-quit-fn (&rest _)
  (rps-doom-quit-p
   (format "%s  %s"
           (propertize (nth (random (length rps-doom-quit-messages))
                            rps-doom-quit-messages)
                       'face '(italic default))
           "Really quit Emacs?")))

(defun rps-quit-but-restart ()
  (interactive)
  (kill-emacs 0 'restart))

(cfg emacs
  (:bind rps-keymap-quit
         "q" #'save-buffers-kill-emacs
         "r" #'rps-quit-but-restart)

  (:opt confirm-kill-emacs #'rps-doom-quit-fn))
